...
export function wireSettingsSaves() {
    ...
    els.savePasswordBtn.addEventListener("click", async () => {
        ...
        const user = udata?.user;
        if (!user) return;
        if (isPasswordChangeBlocked(user)) {
            showMsg(
                els.passwordMsg,
                "Password changes aren’t available for Google- or Discord-only accounts.",
                false
            );
            return;
        }
        if (!user.email) {
            showMsg(els.passwordMsg, "Your account needs an email address to change your password.", false);
            return;
        }
        ...
        if (!cur || !next) {
            showMsg(els.passwordMsg, "Fill in all password fields.", false);
            return;
        }
        if (next.length < 6) {
            showMsg(els.passwordMsg, "New password must be at least 6 characters.", false);
            return;
        }
        if (next !== again) {
            showMsg(els.passwordMsg, "New password and confirmation do not match.", false);
            return;
        }
        ...
        try {
            ...
            els.newPw.value = "";
            els.confirmPw.value = "";
            showMsg(els.passwordMsg, "Password updated successfully.", true);
        } catch (e) {
            console.error(e);
            showMsg(els.passwordMsg, e?.message || "Could not update password.", false);
        } finally {
