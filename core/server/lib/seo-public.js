...
/** `undefined` = legacy (all chapters live). Empty array = nothing live yet. */
function livePublishedChapterIds(payload) {
    if (!payload || typeof payload !== "object") return undefined;
    if (Array.isArray(payload.publishedChapterIds)) {
        return payload.publishedChapterIds.filter((id) => typeof id === "string" && id);
    }
    if (Array.isArray(payload.published_chapter_ids)) {
        return payload.published_chapter_ids.filter((id) => typeof id === "string" && id);
    }
    return undefined;
}

function filterLiveChapters(chapters, publishedIds) {
    const list = Array.isArray(chapters) ? chapters : [];
    if (publishedIds === undefined) return list;
    const allow = new Set(publishedIds);
    return list.filter((ch) => ch && allow.has(ch.id));
}

function firstChapterExcerpt(payload) {
    const chapters = Array.isArray(payload.chapters) ? payload.chapters : [];
    const publishedIds = livePublishedChapterIds(payload);
    let list = filterLiveChapters(chapters, publishedIds);
