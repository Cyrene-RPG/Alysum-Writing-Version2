...
- `apps/` never import other apps. They share through `core/` and `design-system/`.
- `core/` never imports `design-system/` or `apps/` (no DOM, no CSS, no HTML).
- `design-system/` never imports `core/` (look does not know about books).
- Live code never imports `apps/archive/`.
- Main-site does not absorb new product surfaces — those get their own `apps/<name>/`.
...
| --- | --- | --- |
| Folder | kebab-case | `collab-rooms/` |
| Plain file | kebab-case | `conflict-resolver.js` inside `sync-engine/` |
| Shared/flat folder | feature prefix | `sync-conflict-resolver.js` only if files sit flat in `core/` |
| Per-feature folder | drop redundant prefix | `apps/editor/js/pagination.js`, not `editor-pagination.js` |
...
| Custom event | `namespace:action` | `sync:conflict-detected` |
| Storage key | `alysum:feature:...` | `alysum:editor:draft-{id}` |
| BroadcastChannel / caches | same namespace | `alysum:sync:channel` |
