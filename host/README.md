...
Real secrets (service role, LiveKit, IndexNow) live in the host dashboard / a local `.env` that is gitignored. They never go in this folder or in `apps/` / `design-system/` / browser `core/` modules.
...
| `vercel.json` | Vercel |
| `middleware.js` | Vercel (one-line re-export of `host/middleware.js`) |
| `firebase.json` / `.firebaserc` | Firebase CLI |
| `api/` | Vercel functions mount → `core/server/api/` |
